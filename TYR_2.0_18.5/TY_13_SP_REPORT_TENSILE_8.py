...
class TY_13_T8_Ui_MainWindow(object):
...
"color: rgb(0, 0, 0);")
    ...
    def startx(self):
        ...
        results=connection.execute("SELECT DISTINCT PARTY_NAME FROM TEST_MST WHERE TEST_TYPE='Tensile_8'") 
        for x in results:            
            self.comboBox_3.addItem("")
        ...
        self.tableWidget.doubleClicked.connect(self.open_doubleClick_pdf)
        #self.checkBox.clicked.connect(self.check_uncheck_all_records)
        self.pushButton_14_1.clicked.connect(self.open_email_report)
        self.pushButton_13.clicked.connect(self.print_file_new)
        self.pushButton_3.clicked.connect(self.open_comment_popup)
        self.onchage_party()
        self.select_all_tests()
        
    def print_file_new(self):        
    ...
    def onchage_party(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT DISTINCT BATCH_ID FROM TEST_MST where  TEST_TYPE='Tensile_8' and PARTY_NAME='"+self.comboBox_3.currentText()+"'") 
        for x in results:            
    ...
    def select_all_tests(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['TEST NO.','CREATED-ON','Spec. ID','Batch ID','PARTY'])        
         
        connection = sqlite3.connect("tyr.db")  
        results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.SPECIMEN_NAME,BATCH_ID,'"+str(self.party_name)+"' FROM TEST_MST B where B.CREATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"' and TEST_TYPE='Tensile'")                        
    ...
    def open_doubleClick_pdf(self):
        self.pushButton_14_1.setEnabled(True)
        self.pushButton_13.setEnabled(True)
        self.pushButton_3.setEnabled(True)
        self.individual_pdf_report()
        os.system("xpdf ./reports/Reportxx.pdf")    
    #This is for Individual USB reports (Not For Email )
    def individual_pdf_report(self):
        ...
        if(row != -1 ):
            ...
            connection = sqlite3.connect("tyr.db")
            results=connection.execute("SELECT 'Min',printf(\"%.2f\", Min(MAX_FORCE)) ,printf(\"%.2f\", Min(AVG_FORCE)),printf(\"%.2f\", Min(STATIC_COF)),printf(\"%.2f\", Min(KINETIC_COF)),  printf(\"%.2f\", Min(SLEDE_WT_GM)) FROM CYCLES_MST WHERE TEST_ID ='"+str(self.test_id)+"' order by cycle_id Asc")
            ...
            connection = sqlite3.connect("tyr.db")
            results=connection.execute("SELECT 'Max',printf(\"%.2f\", max(MAX_FORCE)) ,printf(\"%.2f\", max(AVG_FORCE)),printf(\"%.2f\", max(STATIC_COF)),printf(\"%.2f\", max(KINETIC_COF)),  printf(\"%.2f\", max(SLEDE_WT_GM)) FROM CYCLES_MST WHERE TEST_ID ='"+str(self.test_id)+"' order by cycle_id Asc")
            ...
            connection = sqlite3.connect("tyr.db")
            results=connection.execute("SELECT 'Avg',printf(\"%.2f\", avg(STATIC_COF)),printf(\"%.2f\", avg(KINETIC_COF)),  printf(\"%.2f\", avg(MAX_FORCE)) ,printf(\"%.2f\", avg(TEST_LENGTH)),printf(\"%.2f\", avg(SLEDE_WT_GM)) FROM CYCLES_MST WHERE TEST_ID ='"+str(self.test_id)+"' order by cycle_id Asc")
            ...
            connection = sqlite3.connect("tyr.db")        
            results=connection.execute("SELECT A.TEST_ID,A.PRODUCT_CODE,A.BATCH_ID,A.TEST_TYPE,A.SPECIMEN_NAME,'NA','NA',A.PARTY_NAME,'NA','NA',A.CREATED_ON,datetime(current_timestamp,'localtime')  FROM TEST_MST A  WHERE   A.TEST_ID ='"+str(self.test_id)+"'")
            for x in results:
            ...
            doc = SimpleDocTemplate('./reports/Reportxx.pdf', rightMargin=10,
                                    leftMargin=30,
                                    topMargin=20,
                                    bottomMargin=20,)
            doc.build(Elements)
