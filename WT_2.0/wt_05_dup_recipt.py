...
class wt_05_Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.groupBox_3.setTitle(_translate("MainWindow", "Vehicle No."))
        ...
        self.label_13.setText(_translate("MainWindow", "Vehicle No.:"))
        ...
        self.label_31.setText(_translate("MainWindow", "Supplier Name :"))
        self.label_32.setText(_translate("MainWindow", "--"))
        self.label_35.setText(_translate("MainWindow", "Transport Name :"))
    ...
    def load_data(self):
        ...
        for x in results:
            print("PRE_PRINT_FLAG:"+str(x[0]))
            if(str(x[0])=='ACTIVE'):
                    self.radioButton_2.setChecked(False)
                    self.radioButton.setChecked(True) #Pre - printed
                    self.radioButton_2.setDisabled(True)
            else:
                    self.radioButton_2.setChecked(True)
                    self.radioButton.setChecked(False) #Pre - printed
        ...
        if(self.listWidget.currentItem().text() != self.listWidget.item(0).text()):
            if(self.list_type=="==== Weighing List ====="):
                ...
                results=connection.execute("SELECT SERIAL_ID_DISPLY,VEHICLE_NO,PARTY_NAME,SUPPLIER_NAME,TRANSPORT_NAME,MATERIAL_NAME,"+
                                   "ifnull(GROSS_WT_DATE,'null') ,GROSS_WT_VAL, ifnull(TARE_WT_DATE,'null') , TARE_WT_VAL ,IFNULL(NET_WEIGHT_VAL,0),(IFNULL(GROSS_WT_RATE,0)+IFNULL(TARE_WT_RATE,0)) as TOTAL_AMT,"+
                ...
                for x in results:                          
                    self.label_17.setText(str(x[10]))  #Net Weight (Kg)
                    self.label_23.setText(str(x[11]))  #Total (Rs)
                    self.label_15.setText(str(x[7]))  #Gross Weight (Kg)
                    #self.label_20.setStyleSheet("color: rgb(0, 0, 255);")
                    #self.label_20.setText(str(x[6][0:10]))                    
                    #self.label_20.setStyleSheet("color: rgb(0, 0, 1);")
                    # self.label_20.setText("       Time: ")
                    #self.label_20.setStyleSheet("color: rgb(0, 0, 255);")
                    self.label_20.setText("<font color=blue> "+str(x[6][0:10])+"</font>             Time:  <font color=blue>"+str(x[6][10:16])+"</font>")
                    #self.label_20.setText(str(x[6][11:16]))  #Rate (Gross):
                    self.label_16.setText(str(x[9]))  #Tare Weight (Kg) 
                    ...
                    self.label_36.setText(str(x[4])) #  Transport Name
                    self.label_32.setText(str(x[3])) #  Auto/mannual
    ...
    def list_vehicles(self):        
        ...
        if(self.radioButton_4.isChecked()):  #vehical No            
            ...
            results=connection.execute("SELECT VEHICLE_NO||' - ('||printf(\"%04d\", SERIAL_ID)||')'||' -'||PARTY_NAME AS SERIAL_ID FROM WEIGHT_MST WHERE (ifnull(round((julianday(CURRENT_TIMESTAMP) - julianday(FIRST_WT_CRTEATED_ON) )),0) < 365)  and PARTY_NAME is not null "+str(self.whr_sql))
            #print("SELECT VEHICLE_NO||' - ('||printf(\"%04d\", SERIAL_ID)||')'||' -'||PARTY_NAME AS SERIAL_ID FROM WEIGHT_MST WHERE (ifnull(round((julianday(CURRENT_TIMESTAMP) - julianday(FIRST_WT_CRTEATED_ON) )),0) < 365)  and PARTY_NAME is not null "+str(self.whr_sql))
            ...
            results=connection.execute("SELECT VEHICLE_NO||' - ('||printf(\"%04d\", SERIAL_ID)||')'||' -'||PARTY_NAME AS SERIAL_ID FROM WEIGHT_MST WHERE (ifnull(round((julianday(CURRENT_TIMESTAMP) - julianday(FIRST_WT_CRTEATED_ON) )),0) < 365) and PARTY_NAME is not null order by serial_id desc ")
        for x in results:        
        ...
        if(self.lineEdit_5.text() != ''):
            if(self.lineEdit_5.text() != ' '):                
                 if(self.radioButton_2.isChecked()):
                    ...
                    with connection:                            
                                    cursor = connection.cursor()
                                    cursor.execute("UPDATE DEVICE_CONF set PRE_PRINT_FLAG=''")
                    connection.commit();
                    connection.close()               
