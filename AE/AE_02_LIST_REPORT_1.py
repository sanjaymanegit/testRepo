...
class AE_02_LIST_2_Ui_MainWindow(object):
...
"border-width:4px;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.radioButton_4.clicked.connect(self.test_id_radiobutt_on_click)
    ...
    def test_id_radiobutt_on_click(self):
        self.frame_2.setDisabled(True)
        self.frame_3.setDisabled(True)
        self.frame_5.setDisabled(True)
    ...
    def list_tests(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['Test ID.','CreatedOn','Spec.\n Counts','Job ID','Batch ID.','Product Name','Comments'])
        ...
        if(self.radioButton.isChecked()):
                ...
                if(self.lineEdit_17.text() != ""):
                    ...
                    results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,(SELECT COUNT(*) as cnt FROM CYCLES_MST A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.JOB_NAME,B.BATCH_ID,B.SPECIMEN_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.TEST_ID='"+str(self.lineEdit_17.text())+"' order by TEST_ID DESC")                        
                else:
                    results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,(SELECT COUNT(*) as cnt FROM CYCLES_MST A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.JOB_NAME,B.BATCH_ID,B.SPECIMEN_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' order by TEST_ID DESC")                        
    ...
    def create_pdf_tensile(self):
        ...
        results=connection.execute("SELECT TEST_ID,CYCLE_NUM,printf(\"%.2f\", A.STG_TENSILE_STRENGTH_KG_CM),printf(\"%.2f\", A.PEAK_LOAD_KG),printf(\"%.2f\", A.CS_AREA_CM) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'AVG','',printf(\"%.2f\", avg(A.STG_TENSILE_STRENGTH_KG_CM)),printf(\"%.2f\", avg(A.PEAK_LOAD_KG)),printf(\"%.2f\", avg(A.CS_AREA_CM)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'MAX','',printf(\"%.2f\", max(A.STG_TENSILE_STRENGTH_KG_CM)),printf(\"%.2f\", max(A.PEAK_LOAD_KG)),printf(\"%.2f\", max(A.CS_AREA_CM)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'MIN','',printf(\"%.2f\", min(A.STG_TENSILE_STRENGTH_KG_CM)),printf(\"%.2f\", min(A.PEAK_LOAD_KG)),printf(\"%.2f\", min(A.CS_AREA_CM)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
