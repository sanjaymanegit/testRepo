...
def dot_matrix_print():
    ...
    connection = sqlite3.connect("fci.db")       
    results=connection.execute("SELECT SERIAL_ID,VEHICLE_NO,ISSUE_ID,IFNULL(TARE_WT_VAL,0),TARE_WT_DATE,IFNULL(GROSS_WT_VAL,0),GROSS_WT_DATE,IFNULL(NET_WEIGHT_VAL,0),IFNULL(ACCPTED_BAGS,0),DRIVER_IN_OUT,CONTRACTOR_NAME,TARGET_STORAGE,MATERIAL_NAME,(SELECT A.ORDER_ID FROM ISSUE_MST A WHERE A.ISSUE_ID=ISSUE_ID) AS ORDER_ID ,(CURR_TRUCK_CNT||'/'||TOTAL_TRUCKS_CNT) as TRUCK_NUM  FROM WEIGHT_MST_FCI_VW  WHERE ISSUE_ID IS NOT NULL AND SERIAL_ID in (SELECT SERIAL_ID from PRINTER_DATA)") 
