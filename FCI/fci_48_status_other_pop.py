...
class fci_48_Ui_MainWindow(object):
    ...
    def select_all_data(self):     
        ...
        if(self.status == "PENDING"):
            results=connection.execute("select SERIAL_ID,PARTY_NAME,VEHICLE_NO,FIRST_WEIGHT_MODE,printf(\"%.3f\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,MATERIAL_NAME,printf(\"%.3f\", IFNULL(NET_WEIGHT_VAL,0)) FROM WEIGHT_MST WHERE BATCH_ISSUE_FLG='OTHER'  and STATUS='FIRST' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
        else:
            results=connection.execute("select SERIAL_ID,PARTY_NAME,VEHICLE_NO,FIRST_WEIGHT_MODE,printf(\"%.3f\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,MATERIAL_NAME,printf(\"%.3f\", IFNULL(NET_WEIGHT_VAL,0)) FROM WEIGHT_MST WHERE BATCH_ISSUE_FLG='OTHER'  and STATUS='SECOND' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
