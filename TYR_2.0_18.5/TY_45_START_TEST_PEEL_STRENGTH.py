...
class TY_45_START_TEST_PEEL_STR_Ui_MainWindow(object):
...
"border-width:4px;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_10.setText(_translate("MainWindow", "Peel Strength"))
...
#         self.comboBox_3.setItemText(2, _translate("MainWindow", "Inch"))
        ...
        self.label_35.setText(_translate("MainWindow", "Test Detail:"))
        ...
        if (len(self.sc_new.arr_p) > 1):            
            ...
            with connection:                
                  ...
                  if( str(self.comboBox_2.currentText()) =="Lb"):
                          ...
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_AVG_FORCE=(SELECT AVG(Y_NUM_LB) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_MAX_FORCE=(SELECT MAX(Y_NUM_LB) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET MIN_FORCE=(SELECT MIN(Y_NUM_LB) FROM STG_GRAPH_MST WHERE  Y_NUM_LB > 0)")   ###
                          
                          
                  elif( str(self.comboBox_2.currentText()) =="N"):
                          cursor.execute("UPDATE GLOBAL_VAR SET STG_PEAK_LOAD_KG=(SELECT MAX(Y_NUM_N) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_AVG_FORCE=(SELECT AVG(Y_NUM_N) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_MAX_FORCE=(SELECT MAX(Y_NUM_N) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET MIN_FORCE=(SELECT MIN(Y_NUM_N) FROM STG_GRAPH_MST WHERE Y_NUM_N > 0)")   ###
                          
                  elif( str(self.comboBox_2.currentText()) =="KN"):
                          cursor.execute("UPDATE GLOBAL_VAR SET STG_PEAK_LOAD_KG=(SELECT MAX(Y_NUM_KN) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_AVG_FORCE=(SELECT AVG(Y_NUM_KN) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET COF_MAX_FORCE=(SELECT MAX(Y_NUM_KN) FROM STG_GRAPH_MST)")   ###
                          cursor.execute("UPDATE GLOBAL_VAR SET MIN_FORCE=(SELECT MIN(Y_NUM_KN) FROM STG_GRAPH_MST WHERE Y_NUM_KN > 0)")   ###
    ...
    def create_pdf_tensile(self):
        ...
        for x in results:
            summary_data=[["Tested Date-Time: ",str(x[10]),"Test No: ",str(x[0])],["Test Detail : ",str(x[1]),"Batch ID: ",str(x[2])],["Product Name:  ",str(x[4])," Shape:",str(x[9])],["Test Type:",str(x[3]),"Test Method: ",str(x[8])],["Customer Name :",str(x[7]),"Test Speed (mm/min) :",str(x[5])],["Product Width(mm):",str(x[6]),"Report Date-Time: ",str(x[11])],["Tested By :", str(self.tested_by),"",""]]
...
if __name__ == "__main__":
    ...
    ui = TY_45_START_TEST_PEEL_STR_Ui_MainWindow()
