...
class TY_35_LIST_Ui_MainWindow_COMP_3(object):
...
#         self.kgCm2_toMPA=float(0.0980665)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_15.setText(_translate("MainWindow", "Part No:"))
        #self.label_15.setDisabled(True)
        self.comboBox_2.setDisabled(True)
        self.label_16.setText(_translate("MainWindow", "Part No:"))
...
#         connection.close()
    ...
    def load_jobname_1(self):
        ...
        self.comboBox_2.clear()
        connection = sqlite3.connect("tyr.db")       
        results=connection.execute("SELECT DISTINCT PART_NO FROM TEST_MST WHERE TEST_TYPE='"+str(self.new_test_name)+"' order by TEST_ID DESC")         
    ...
    def list_tests(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['Test ID.','CreatedOn','Customer Name','Spec.Counts','Batch ID.','Part Name','Comments'])
        ...
        if(self.radioButton.isChecked()):
                ...
                results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where date(B.CREATED_ON) between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"' and B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.PARTY_NAME = '"+str(self.comboBox.currentText())+"'  order by TEST_ID DESC")                        
                
        elif(self.radioButton_2.isChecked()):
                print("Search by Tested By.....")
                results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.TESTED_BY = '"+str(self.comboBox_4.currentText())+"' order by TEST_ID DESC")                        
        elif(self.radioButton_3.isChecked()):
                print("by batch id -select")
                results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.BATCH_ID = '"+str(self.comboBox_6.currentText())+"' order by TEST_ID DESC ")                        
        elif(self.radioButton_4.isChecked()):
                if(self.lineEdit_17.text() != ""):
                    print("by Test Id / Serial No -select")
                    results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.TEST_ID='"+str(self.lineEdit_17.text())+"' order by TEST_ID DESC")                        
                else:
                    results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' order by TEST_ID DESC")                        
            
        elif(self.label_15.isChecked()):
                print("by Job id -select")
                results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.PARTY_NAME,(SELECT COUNT(DISTINCT SPEC_ID) as cnt FROM TEST_DATA A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.BATCH_ID,B.PART_NAME,COMMENTS FROM TEST_MST B where B.TEST_TYPE='"+str(self.new_test_name)+"' and  B.PART_NO = '"+str(self.comboBox_2.currentText())+"' order by TEST_ID DESC ")                        
        else:
    ...
    def create_pdf_TEAR(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT CYCLE_NUM,printf(\"%.2f\", A.PEAK_LOAD_KG) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
    ...
    def create_pdf_CLD2(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT CYCLE_NUM,printf(\"%.2f\", A.LOAD_POINT_1),printf(\"%.2f\", A.LOAD_POINT_2) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'AVG',printf(\"%.2f\", avg(A.LOAD_POINT_1)),printf(\"%.2f\", avg(A.LOAD_POINT_2)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'MAX',printf(\"%.2f\", max(A.LOAD_POINT_1)),printf(\"%.2f\", max(A.LOAD_POINT_2)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'MIN',printf(\"%.2f\", min(A.LOAD_POINT_1)),printf(\"%.2f\", min(A.LOAD_POINT_2)) FROM CYCLES_MST A WHERE A.TEST_ID IN (SELECT TEST_ID FROM TEST_IDS)") 
        for x in results:
                data2.append(x)
        connection.close()
    ...
    def create_pdf_COMPRESSION_3(self):
        ...
        Elements=[Title,Title2,Spacer(1,12),Spacer(1,12),Spacer(1,12),Spacer(1,12),f2,Spacer(1,12),blank,Spacer(1,12),Spacer(1,12),Spacer(1,12)]
...
class PlotCanvas(FigureCanvas):
    ...
    def plot(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT GRAPH_ID FROM CYCLES_MST WHERE TEST_ID IN (SELECT TEST_ID FROM TEST_IDS) order by GRAPH_ID") 
        for x in results:
...
#         connection.close()
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT LAST_UNIT_LOAD,LAST_UNIT_DISP,GRAPH_SCAL_X_LENGTH,GRAPH_SCAL_Y_LOAD from TEST_MST  WHERE TEST_ID IN (SELECT TEST_ID FROM TEST_IDS) ") 
        for x in results:
              self.last_load_unit=str(x[0])
              self.last_disp_unit=str(x[1])
...
if __name__ == "__main__":
    ...
    ui = TY_35_LIST_Ui_MainWindow_COMP_3()
