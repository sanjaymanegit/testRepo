...
def report_print():
    ...
    try:
        ...
        printer.text("     \n\r")
        printer.text("|------------------------------------------------------------------------\n\r")
        printer.text("| #      |  Vehical No  |Gross Wt|Gross-Date  |Time   |Tare Wt | Tare-Date  |Time   | Net Wt | Total   |Party Name |\n\r")
        printer.text("|------------------------------------------------------------------------\n\r")        
        for i in range(len(SER_ID)):
                printer.text("| "+str(SER_ID[i]).zfill(6)+" | "+str(VEHICLE_NO[i])+" | "+str(GROSS_WT_VAL[i]).zfill(6)+" | "+str(GROSS_WT_DATE[i])[0:10]+" | "+str(GROSS_WT_DATE[i])[11:16]+" | "+str(TARE_WT_VAL[i]).zfill(6)+" | "+str(TARE_WT_DATE[i])[0:10]+" | "+str(TARE_WT_DATE[i])[11:16]+" | "+str(NET_WEIGHT_VAL[i]).zfill(6)+" | "+str(TOTAL_AMT[i]).zfill(7)+" | "+str(PARTY_NAME[i])+" |\n\r")
                printer.text("|                                                                  \n\r") 
        printer.text("|------------------------------------------------------------------------\n\r")
