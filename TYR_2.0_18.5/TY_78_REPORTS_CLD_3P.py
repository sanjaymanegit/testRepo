...
class TY_78_Ui_MainWindow(object):
...
"border-width:4px;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_10.setText(_translate("MainWindow", "CLD-3P-(Report)"))
        ...
        self.label_23.setText(_translate("MainWindow", "Spec.Type:"))
...
#               self.comboBox_3.setItemText(self.i,"Cm")
        ...
        self.label_51.setText("("+str(self.comboBox_2.currentText())+")")
    ...
    def load_data(self):
        ...
        hh=0
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT DEFLCTION FROM TEST_DATA WHERE TEST_ID ='"+str(self.test_id)+"' and SPEC_Id=1 ORDER BY ID ASC") 
        for x in results:            
            if(hh==0):
                 print("Rec...1:"+str(x[0]))
                 self.lineEdit_20.setText(str(x[0]))
                 
            elif(hh==1):
                 print("Rec...2:"+str(x[0]))
                 self.lineEdit_21.setText(str(x[0]))
            else:
                 self.lineEdit_22.setText(str(x[0]))
            hh=int(hh)+1    
        connection.close()
...
#                         cursor.execute("INSERT INTO STG_TEST_DATA(DEFLCTION)VALUES('"+str(self.lineEdit_24.text())+"')")
    ...
    def show_grid_data_Tear(self):
        ...
        connection = sqlite3.connect("tyr.db")
        self.tableWidget.setHorizontalHeaderLabels([' Cycle.ID ',' Load_@'+str(self.lineEdit_20.text())+'mm \n ('+str(self.comboBox_2.currentText())+') ',' Load_@'+str(self.lineEdit_21.text())+'mm \n ('+str(self.comboBox_2.currentText())+') ',' Load_@'+str(self.lineEdit_22.text())+'mm \n ('+str(self.comboBox_2.currentText())+')','rec_id'])
        results=connection.execute("SELECT CYCLE_ID,printf(\"%.2f\", LOAD_1),printf(\"%.2f\", LOAD_2),printf(\"%.2f\", LOAD_3),ID FROM LOAD_DATA WHERE TEST_ID = '"+self.test_id+"' order by ID ASC")
    ...
    def create_pdf_Tear(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT printf(\"%.2f\", CYCLE_ID),printf(\"%.2f\", LOAD_1),printf(\"%.2f\", LOAD_2),printf(\"%.2f\", LOAD_3) FROM LOAD_DATA WHERE TEST_ID = '"+self.test_id+"' order by ID ASC")
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'Avg.',printf(\"%.2f\", avg(LOAD_1)),printf(\"%.2f\", avg(LOAD_2)),printf(\"%.2f\", avg(LOAD_3)) FROM LOAD_DATA WHERE TEST_ID = '"+self.test_id+"' order by ID ASC")
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'Max.',printf(\"%.2f\", max(LOAD_1)),printf(\"%.2f\", max(LOAD_2)),printf(\"%.2f\", max(LOAD_3)) FROM LOAD_DATA WHERE TEST_ID = '"+self.test_id+"' order by ID ASC")
        for x in results:
                data2.append(x)
        connection.close()
        
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT 'Min.',printf(\"%.2f\", min(LOAD_1)),printf(\"%.2f\", min(LOAD_2)),printf(\"%.2f\", min(LOAD_3)) FROM LOAD_DATA WHERE TEST_ID = '"+self.test_id+"' order by ID ASC")
        for x in results:
                data2.append(x)
        connection.close()
