...
#                     )
    ...
    def delete_cycle(self):
        print("Inside..xxxx"+str(self.test_type_for_flexural))
        if(self.test_type_for_flexural=="Flexural"):
            ...
            if(int(self.cycle_id) > 0):
                ...
                pass
        elif(self.test_type_for_flexural=="TENSILE_GASKET_SQUARE"      or self.test_type_for_flexural=="TENSILE_GASKET_ROUND" ):
    ...
    def create_pdf_tensile(self):
        ...
        if(self.last_load_unit == "MPa" and self.last_disp_unit=="Mm"):
                ...
                connection = sqlite3.connect("tyr.db")
                results=connection.execute("SELECT CYCLE_NUM,printf(\"%.4f\", CS_AREA*0.01),printf(\"%.2f\", PEAK_LOAD_KG),printf(\"%.2f\", E_AT_PEAK_LOAD_MM*0.1),printf(\"%.2f\", PRC_E_AT_PEAK),printf(\"%.2f\", E_AT_BREAK_MM),printf(\"%.2f\", PRC_E_AT_BREAK) FROM CYCLES_MST WHERE TEST_ID IN (SELECT TEST_ID FROM GLOBAL_VAR)")
                for x in results:
                        data.append(x)
                connection.close()        
                ...
                connection = sqlite3.connect("tyr.db")            
                results=connection.execute("SELECT 'AVG',printf(\"%.4f\", avg(CS_AREA*0.01)),printf(\"%.2f\", avg(PEAK_LOAD_KG)),printf(\"%.2f\", avg(E_AT_PEAK_LOAD_MM*0.1)),printf(\"%.2f\", avg(PRC_E_AT_PEAK)),printf(\"%.2f\", avg(E_AT_BREAK_MM)),printf(\"%.2f\", avg(PRC_E_AT_BREAK)) FROM CYCLES_MST WHERE TEST_ID IN (SELECT TEST_ID FROM GLOBAL_VAR) LIMIT 1")
                for x in results:
                        data.append(x)
                connection.close()
                    
                connection = sqlite3.connect("tyr.db")
                results=connection.execute("SELECT 'MAX',printf(\"%.4f\", max(CS_AREA*0.01)),printf(\"%.2f\", max(PEAK_LOAD_KG)),printf(\"%.2f\", max(E_AT_PEAK_LOAD_MM*0.1)),printf(\"%.2f\", max(PRC_E_AT_PEAK)),printf(\"%.2f\", max(E_AT_BREAK_MM)),printf(\"%.2f\", max(PRC_E_AT_BREAK)) FROM CYCLES_MST WHERE TEST_ID IN (SELECT TEST_ID FROM GLOBAL_VAR) LIMIT 1")
                for x in results:
                        data.append(x)
                connection.close()
                    
                connection = sqlite3.connect("tyr.db")
                results=connection.execute("SELECT 'MIN',printf(\"%.4f\", min(CS_AREA*0.01)),printf(\"%.2f\", min(PEAK_LOAD_KG)),printf(\"%.2f\", min(E_AT_PEAK_LOAD_MM*0.1)),printf(\"%.2f\", min(PRC_E_AT_PEAK)),printf(\"%.2f\", min(E_AT_BREAK_MM)),printf(\"%.2f\", min(PRC_E_AT_BREAK))  FROM CYCLES_MST WHERE TEST_ID IN (SELECT TEST_ID FROM GLOBAL_VAR) LIMIT 1")
                for x in results:
                        data.append(x)
                connection.close()
