...
class fci_47_Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        for x in results:            
            self.label.setText("ISSUE - "+str(x[0]))
            self.status=str(x[0])
    ...
    def select_all_data(self):     
        ...
        self.tableWidget.setHorizontalHeaderLabels(['Slip No.','Order Id','Vehicle No.','Material','No.Of.Bags','Net.Wt','First Weight Type','First Weight','First Weight On','Second Weight Type','Second Weight','Second Weight On'])        
           
        connection = sqlite3.connect("fci.db")
        if(self.status == "PENDING"):
                results=connection.execute("select SERIAL_ID,(SELECT A.ORDER_ID FROM ISSUE_MST A WHERE A.ISSUE_ID=ISSUE_ID) as ISSUE_ID,VEHICLE_NO,MATERIAL_NAME,printf(\"%03d\", IFNULL(PROPOSED_BAGS,0)),printf(\"%6d\", IFNULL(NET_WEIGHT_VAL,0)),FIRST_WEIGHT_MODE,printf(\"%6d\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,SECOND_WT_MODE,printf(\"%6d\", IFNULL(SECOND_WT_VAL,0)) ,SECOND_WT_CREATED_ON FROM WEIGHT_MST WHERE BATCH_ISSUE_FLG='ISSUE'   and STATUS='FIRST' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
        else:
                results=connection.execute("select SERIAL_ID,(SELECT A.ORDER_ID FROM ISSUE_MST A WHERE A.ISSUE_ID=ISSUE_ID) as ISSUE_ID,VEHICLE_NO,MATERIAL_NAME,printf(\"%03d\", IFNULL(PROPOSED_BAGS,0)),printf(\"%6d\", IFNULL(NET_WEIGHT_VAL,0)),FIRST_WEIGHT_MODE,printf(\"%6d\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,SECOND_WT_MODE,printf(\"%6d\", IFNULL(SECOND_WT_VAL,0)) ,SECOND_WT_CREATED_ON  FROM WEIGHT_MST WHERE BATCH_ISSUE_FLG='ISSUE'   and STATUS='SECOND' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
