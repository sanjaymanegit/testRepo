...
def report_print():
    ...
    connection = sqlite3.connect("wt.db")       
    results=connection.execute("SELECT REPORT_FROM_DATE ,REPORT_TO_DATE,FILTER_COL,ALL_FLAG,total_amount,FILTER_COL_VAL from PRINTER_DATA")
    for x in results:
         from_date=str(x[0])
         to_date=str(x[1])
    ...
    if(str(all_flag)=='ACTIVE'):
            ...
            if(filter_col_name == "Party Name"):
                whr_str=" WHERE strftime('%Y-%m-%d',FIRST_WT_CRTEATED_ON)  between '"+str(from_date)+"' and '"+str(to_date)+"' AND PARTY_NAME='"+str(filter_col_val)+"' limit 5000 "
       
            elif(filter_col_name == "Supplier Name"):
                whr_str=" WHERE strftime('%Y-%m-%d',FIRST_WT_CRTEATED_ON)  between '"+str(from_date)+"' and '"+str(to_date)+"' AND SUPPLIER_NAME='"+str(filter_col_val)+"' limit 5000 "
        
            elif(filter_col_name == "Transport Name"):
                whr_str=" WHERE strftime('%Y-%m-%d',FIRST_WT_CRTEATED_ON)  between '"+str(from_date)+"' and '"+str(to_date)+"' AND TRANSPORT_NAME='"+str(filter_col_val)+"' limit 5000 "
    ...
    if(filter_col_name != "collection_report"):
                ...
                try:
                    ...
                    try:
                        ...
                        if(str(all_flag)!='ACTIVE'):
                                    printer.text(str(filter_col_val)+" : "+str(filter_col_val)+" \n\r")
                        
                        printer.text("     \n\r")
                        printer.text("|-------------------------------------------------------------------\n\r")
                        printer.text("| #      |  Vehicle No  |Gross Wt|Gross-Date  |Tare Wt | Tare-Date  \n\r")
                        printer.text("|                                |Time                 | Time       \n\r")
                        printer.text("|-------------------------------------------------------------------\n\r")        
                        for i in range(len(SER_ID)):
                                    printer.text("| "+str(SER_ID[i]).zfill(6)+" | "+str(VEHICLE_NO[i])+" | "+str(GROSS_WT_VAL[i]).zfill(6)+" | "+str(GROSS_WT_DATE[i]).zfill(10)[0:10]+" | "+str(TARE_WT_VAL[i]).zfill(6)+" | "+str(TARE_WT_DATE[i]).zfill(10)[0:10]+" |\n\r")
                                    printer.text("|                                 "+str(GROSS_WT_DATE[i])[11:16]+"                      "+str(TARE_WT_DATE[i]).zfill(6)[11:16]+"\n\r")
                                    printer.text("   Net Wt : "+str(NET_WEIGHT_VAL[i]).zfill(6)+" Charges (Rs) : "+str(TOTAL_AMT[i]).zfill(7)+" \n\r")
                                    printer.text("|-------------------------------------------------------------------\n\r")
                ...
                connection = sqlite3.connect("wt.db")  
                results=connection.execute("SELECT SERIAL_ID_DISPLY,VEHICLE_NO,(IFNULL(GROSS_WT_RATE,0)+ IFNULL(TARE_WT_RATE,0)) as TOTAL_AMT FROM WEIGHT_MST_VW "+str(whr_str)) 
                ...
                try:
                    ...
                    try:
                        ...
                        printer.text("     \n\r")
                        printer.text("|-------------------------------------------------------------------\n\r")
                        printer.text("| Serial No    |  Vehicle No  | Changes(Rs) \n\r")                     
                        printer.text("|-------------------------------------------------------------------\n\r")
                        for i in range(len(SER_ID)):
                               printer.text("| "+str(SER_ID[i]).zfill(6)+"    |  "+str(VEHICLE_NO[i])+"  | "+str(TOTAL_AMT[i]).zfill(7)+" \n\r")
                        printer.text("*******************  Thanking You   ********************\n\r")             
                        printer.lf()
                    except ValueError:
                            print("Error")
