...
class TY_13_Ui_MainWindow(object):
...
"color: rgb(0, 0, 0);")
    ...
    def retranslateUi(self, MainWindow):
        ...
        #self.comboBox_5.addItem("")
        self.comboBox_5.setItemText(0, _translate("MainWindow", "gm/mm"))
    ...
    def startx(self):
        ...
        results=connection.execute("SELECT DISTINCT PARTY_NAME FROM TEST_MST WHERE SPECIMEN_NAME='COF_SPECIMEN'") 
        for x in results:            
            self.comboBox_3.addItem("")
        ...
        self.pushButton_3.clicked.connect(self.open_sample_pdf)
    ...
    def onchage_party(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT DISTINCT PRODUCT_CODE FROM TEST_MST where  PARTY_NAME='"+self.comboBox_3.currentText()+"'") 
        for x in results:            
    ...
    def select_all_tests(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['TEST NO.','NO.OF.CYCLES.','CREATED-ON','TEST-TYPE','LOT NO','PRODUCT CODE','PARTY'])        
        ...
        results=connection.execute("SELECT B.TEST_ID,(SELECT COUNT(*) as cnt FROM CYCLES_MST A WHERE A.TEST_ID=B.TEST_ID) as CYCLES_CNT,B.CREATED_ON,B.TEST_TYPE,B.LOT_NO,B.PRODUCT_CODE,'"+str(self.party_name)+"' FROM TEST_MST B where B.CREATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"' and SPECIMEN_NAME='COF_SPECIMEN'")                        
    ...
    def load_reports(self):        
            ...
            for x in range(len(self.test_ids)):
                    ...
                    connection = sqlite3.connect("tyr.db")  
                    results=connection.execute("SELECT TEST_TYPE FROM TEST_MST WHERE TEST_ID  = '"+str(self.test_id)+"'")
                    for x in results:
    ...
    def create_report_pdf(self):
        ...
        for x in results:
            summary_data=[["Tested Date: ",str(x[10]),"Test No: ",str(x[0])],["Job Name : ",str(x[1]),"Batch ID: ",str(x[2])],["Specimen Name:  ",str(x[4]),"Specmen Shape:",str(x[6])],["Test Type:",str(x[3]),"Specmen Specs:",str(x[6])],["Party Name :",str(x[7]),"Test Speed :",str(x[5])],["Guage Length(mm):",str(x[6]),"Report Date: ",str(x[11])],["Tested By :", "","",""]]
    ...
    #This is report for all data consolidate reports
    def create_pdf_cof(self):        
        ...
        Elements=[Title,Title2,Spacer(1,12),Spacer(1,12),Spacer(1,12),f2,Spacer(1,12),Spacer(1,12),Spacer(1,12),comments,blank,Spacer(1,12),Spacer(1,12),footer_2,Spacer(1,12)]
        
        #Elements.append(f1,Spacer(1,12))        
        #Elements.append(f2,Spacer(1,12))
        
        doc = SimpleDocTemplate('./reports/sr_report.pdf', rightMargin=10,
                                leftMargin=30,
                                topMargin=20,
                                bottomMargin=20,)
        doc.build(Elements)
