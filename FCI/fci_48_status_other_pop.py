...
class fci_48_Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.frame.setGeometry(QtCore.QRect(30, 30, 921, 321))
    ...
    def select_all_data(self):     
        ...
        self.tableWidget.setHorizontalHeaderLabels(['Slip No.','Party Name','Vehicle No.','First Weight Type','First Weight','First Weight On','Material','Net.Wt'])        
           
        connection = sqlite3.connect("fci.db")
        if(self.status == "PENDING"):
            results=connection.execute("select SERIAL_ID,PARTY_NAME,VEHICLE_NO,FIRST_WEIGHT_MODE,printf(\"%.3f\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,MATERIAL_NAME,printf(\"%.3f\", IFNULL(NET_WEIGHT_VAL,0)) FROM WEIGHT_MST WHERE ISSUE_ID IS  NULL AND  BATCH_ID IS NULL and STATUS='FIRST' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
        else:
            results=connection.execute("select SERIAL_ID,PARTY_NAME,VEHICLE_NO,FIRST_WEIGHT_MODE,printf(\"%.3f\", IFNULL(FIRST_WEIGHT_VAL,0)) ,FIRST_WT_CRTEATED_ON,MATERIAL_NAME,printf(\"%.3f\", IFNULL(NET_WEIGHT_VAL,0)) FROM WEIGHT_MST WHERE ISSUE_ID IS  NULL AND  BATCH_ID IS NULL and STATUS='SECOND' and FIRST_WT_CRTEATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"'")                        
