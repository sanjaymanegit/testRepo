...
"background-color: rgb(0, 0, 0);")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_53.setText(_translate("MainWindow", "BUY"))
    ...
    def load_1st_wt_vehicles(self):
        ...
        results=connection.execute("SELECT VEHICLE_NO||' - ('||printf(\"%04d\", SERIAL_ID)||')' AS SERIAL_ID FROM WEIGHT_MST WHERE STATUS='FIRST' AND BATCH_ISSUE_FLG='BUY'")       
        for x in results:
    ...
    def load_2nd_wt_vehicles(self):
        self.listWidget_2.clear()
        connection = sqlite3.connect("fci.db")
        results=connection.execute("SELECT VEHICLE_NO||' - ('||printf(\"%04d\", SERIAL_ID)||')' AS SERIAL_ID FROM WEIGHT_MST WHERE STATUS='SECOND' AND BATCH_ISSUE_FLG='BUY'")       
        for x in results:
               item= QtWidgets.QListWidgetItem(str(x[0]))
               item.setBackground(QtGui.QColor("light blue"))
                                             ...
                                             with connection:                            
                                                        ...
                                                        cursor.execute("INSERT INTO WEIGHT_MST(VEHICLE_NO,MATERIAL_NAME,STATUS,FIRST_WEIGHT_MODE,FIRST_WEIGHT_VAL,FIRST_WT_CRTEATED_ON,WEIGHT_TYPE,ACCPTED_BAGS,AVG_BAG_WT,REMARK,"
                                                            +"DRIVER_IN_OUT,DEVICE_LOCATION_TYPE,ACCPTED_BAGS,DEVICE_ID,CREATED_BY,MANNUAL_INS_FLG,PARTY_NAME,AMOUNT,PHONE_NO,BATCH_ISSUE_FLG)"
                                                                       ...
                                                                       +self.avg_bag_wt+"','"+self.remark+"','"+str(self.driver_in_out)+"','"+str(self.device_location_type)+"','"+self.accepted_bags+"','"+str(self.device_id)+"','"+str(self.login_user_id)+"','"+str(self.manual_ins_flg)+"','"+str(self.parties)+"','"+str(self.charges)+"','"+str(self.mobile_no)+"','BUY')")
                                                                      
                                             #self.reset_fun()
                                             self.label_56.setText("Successfully Saved First Weight.")
                                             self.log_audit("Weighing","BUY-Loaded First Weight. slip No:"+str(int(self.label_19.text())))
                                             self.label_56.show()  
                                             connection.commit();
                                             connection.close()
                                             print("Data Inserted !!!!")
                                 ...
                                 if(self.label_29.text() != self.label_37.text()):
                                     if(float(self.label_45.text()) > 0 ):
                                         ...
                                         if(str(self.goAhead) == 'Yes'):
                                                 ...
                                                 self.log_audit("Weighing","BUY-Loaded Second Weight. slip No:"+str(int(self.label_19.text())))
                                                 self.label_56.show()
                                                 self.load_1st_wt_vehicles()
                                                 self.load_2nd_wt_vehicles()
                                                 self.pushButton_5.setDisabled(True)
