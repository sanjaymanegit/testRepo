...
# Form implementation generated from reading ui file 'wt_14_ser_weighing_mode.ui'
#
...
class wt_14_Ui_MainWindow(object):
...
"color: rgb(170, 0, 0);")
        ...
        self.checkBox.setGeometry(QtCore.QRect(90, 60, 201, 31))
        self.checkBox.setObjectName("checkBox")
        self.checkBox_2 = QtWidgets.QCheckBox(self.groupBox_2)
        self.checkBox_2.setGeometry(QtCore.QRect(380, 60, 171, 31))
        self.checkBox_2.setObjectName("checkBox_2")
        self.checkBox_3 = QtWidgets.QCheckBox(self.groupBox_2)
        self.checkBox_3.setGeometry(QtCore.QRect(640, 60, 171, 31))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_19.setText(_translate("MainWindow", "Weighing Mode Setting"))
        ...
        self.groupBox_2.setTitle(_translate("MainWindow", "Set Weighing Mode"))
        self.pushButton_5.setText(_translate("MainWindow", "Save"))
        self.checkBox.setText(_translate("MainWindow", "Regular Weighing (default)"))
        self.checkBox_2.setText(_translate("MainWindow", "Public Weighing"))
        self.checkBox_3.setText(_translate("MainWindow", "Mutli-Weighing"))
    ...
    def load_data(self):      
        ...
        results=connection.execute("SELECT WEIGHING_MODE_FLG,WEIGHING_PUBLIC_FLG,WEIGHING_MULTI_FLG FROM SERIVES_SET") 
        for x in results:            
            if(str(x[0])=='ACTIVE'):
    ...
    def save_data(self):        
        if(self.checkBox.isChecked()):
            self.weighing_mode='ACTIVE'
        else:
            self.weighing_mode='DEACTIVE'
        
        if(self.checkBox_2.isChecked()):
            self.weighing_public='ACTIVE'
        else:
            self.weighing_public='DEACTIVE'
            
        if(self.checkBox_3.isChecked()):
            self.weighing_multi='ACTIVE'
        else:
            self.weighing_multi='DEACTIVE'    
                        
        connection = sqlite3.connect("wt.db")          
        with connection:        
                cursor = connection.cursor()                    
                cursor.execute("UPDATE SERIVES_SET SET WEIGHING_MODE_FLG = '"+str(self.weighing_mode)+"',WEIGHING_PUBLIC_FLG='"+str(self.weighing_public)+"',WEIGHING_MULTI_FLG='"+str(self.weighing_multi)+"'") 
        connection.commit();
        connection.close()
