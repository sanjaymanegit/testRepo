...
class wt_19_Ui_MainWindow(object):
...
"color: rgb(170, 0, 0);")
    ...
    def login_page(self):
        ...
        results=connection.execute("SELECT PWD,xxx FROM SERVICES_MST WHERE SERVICE_NAME = 'FACTORY_RESET' AND STATUS = 'ACTIVE'") 
        for x in results:
            print("key:"+str(x[1]))
            key=str(x[1])
            val=str(x[0])
    ...
    def clean_data(self):
        ...
        if(self.checkBox_2.isChecked()):            
            ...
            with connection:        
                        cursor = connection.cursor()                    
                        cursor.execute("UPDATE SERIVES_SET SET SS_PWD='SS_1234' ")
                        cursor.execute("UPDATE USER_RIGHT_SET SET UR_PWD='UR_1234'")                        
            connection.commit();
            connection.close()
            
            key = Fernet.generate_key()
            cipher_suite = Fernet(key)
            b = bytes(str("12345"), 'utf-8')
            ...
            with connection:        
                        cursor = connection.cursor()                    
                        cursor.execute("UPDATE SERVICES_MST SET PWD = '"+str(cipher_text,'utf-8')+"', xxx='"+str(key,'utf-8')+"'") 
            connection.commit();
            connection.close()
