...
function f1()
{	
document.client_list_form.target = "bottomframe2";
...
var input_flag = confirm(" Confirm the Client which is going to delete : "+document.client_list_form.elements['edit_radio'].value);
	if(input_flag == true)
	{
		...
		document.client_list_form.target = "bottomframe";
...
if (del_flag != null && del_flag.equals("Yes"))
{
 ...
 sql="DELETE FROM client_dtls WHERE client_id='"+client_id+"'"; 
 //out.println("sql  :"+sql); 
...
<h3>  Clients </h3>   
<div class="btn-group">
  <button type="button" onclick="f1()"> Add Client</button> 
  <button type="button" onclick="f2()">Edit Client</button>  
  <button type="button" onclick="f3()">Delete Client</button>  
</div>  
  <br> 
<table border=0 >
<tr bgcolor="grey" >
...
<%
 try{
  statement=con.createStatement();
  sql ="SELECT client_id,client_name,client_desc FROM client_dtls";
  resultSet = statement.executeQuery(sql);
 while(resultSet.next()){
 %>
<tr>
<td><input type="radio" name="edit_radio" value="<%=resultSet.getString("client_id")%>"><%=resultSet.getString("client_name")%></td>
<td><%=resultSet.getString("client_desc")%></td>
</tr>
<%
}
 connection.close();
