...
def report_print():
    ...
    connection = sqlite3.connect("fci.db")        
    results=connection.execute("select ISSUE_ID ,TOTAL_TRUCKS, TOTAL_NET_WT, TOTAL_ACCEPTED_BAGS,(SELECT A.ORDER_ID FROM ISSUE_MST A WHERE A.ISSUE_ID=ISSUE_ID) AS ORDER_ID  from ISSUE_LIST_VW "+str(whr_sql)) 
    for x in results:            
    ...
    try:
        ...
        for i in range(len(ISSUE_IDS)):
                    printer.text("Order Id        : "+str(ORDER_IDS[i])+"                     Total Trucks    : "+str(TOTAL_TRUCKS[i]).zfill(4)+" \n\r")
                    ...
                    connection = sqlite3.connect("fci.db")        
                    results=connection.execute("select SERIAL_ID,VEHICLE_NO,ACCPTED_BAGS,NET_WEIGHT_VAL,TARGET_STORAGE from WEIGHT_MST_FCI_VW WHERE ISSUE_ID='"+str(ISSUE_IDS[i])+"'") 
                    for x in results: 
