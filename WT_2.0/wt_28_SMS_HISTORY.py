...
class wt_28_Ui_MainWindow(object):
    ...
    def list_report_data(self):
        ...
        if(self.whr_str != ""):
            results=connection.execute("select PHONE_NO,STATUS,CREATED_ON,ERROR_MSG,SERIAL_ID,SMS_MSG,SMS_ID from SMS_HISTORY where SMS_TYPE='CUSTOMER'  "+str(self.whr_str)+ " order by CREATED_ON desc")
        else:
            results=connection.execute("select PHONE_NO,STATUS,CREATED_ON,ERROR_MSG,SERIAL_ID,SMS_MSG,SMS_ID from SMS_HISTORY where SMS_TYPE='CUSTOMER'  "+str(self.whr_str)+ " order by CREATED_ON desc")
