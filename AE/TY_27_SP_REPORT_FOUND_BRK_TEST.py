...
from TY_28_REPORT_FOUND_BRK_TEST import TY_28_Ui_MainWindow
...
class TY_27_FBST_Ui_MainWindow(object):
...
"color: rgb(0, 0, 0);")
    ...
    def startx(self):
        ...
        results=connection.execute("SELECT DISTINCT PARTY_NAME FROM TEST_MST WHERE TEST_TYPE='FBST'") 
        for x in results:            
            self.comboBox_3.addItem("")
    ...
    def onchage_party(self):
        ...
        connection = sqlite3.connect("tyr.db")
        results=connection.execute("SELECT DISTINCT BATCH_ID FROM TEST_MST where  TEST_TYPE='FBST' and PARTY_NAME='"+self.comboBox_3.currentText()+"'") 
        for x in results:            
    ...
    def select_all_tests(self):
        ...
        results=connection.execute("SELECT B.TEST_ID,B.CREATED_ON,B.SPECIMEN_NAME,B.BATCH_ID,B.PARTY_NAME,B.TEST_TYPE FROM TEST_MST B where B.CREATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"' and TEST_TYPE ='FBST'")                        
        ...
        with connection:        
                ...
                cursor.execute("INSERT INTO TEST_IDS SELECT B.TEST_ID,B.TEST_TYPE  FROM TEST_MST B  where B.CREATED_ON between '"+str(self.from_dt)+"' and '"+str(self.to_dt)+"' and SPECIMEN_NAME='FBST'") 
        connection.commit();
        connection.close()
