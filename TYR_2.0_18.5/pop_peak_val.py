...
"background-color: rgb(226, 239, 255);")
    ...
    def show_single_graph_data(self):        
        ...
        if(row > -1):
                    ...
                    self.tableWidget.setHorizontalHeaderLabels(['ID','Peak Val','Test Method'])  
                          
                    self.tableWidget.setColumnWidth(0, 100)
                    self.tableWidget.setColumnWidth(1, 100)
                    self.tableWidget.setColumnWidth(2, 100)
                    connection = sqlite3.connect("tyr.db")                     
                    print("SELECT printf(\"%.2f\", SQ_NO),printf(\"%.2f\", PEAK_VAL), TEST_METHOD_TYPE FROM PEAK_MST WHERE PEAk_LIST_ID IN (SELECT MAX(PEAK_LIST_ID) FROM TEST_DATA WHERE GRAPH_ID = '"+str(self.g[row])+"') ")
                    
                    results=connection.execute("SELECT printf(\"%.2f\", SQ_NO),printf(\"%.2f\", PEAK_VAL), TEST_METHOD_TYPE FROM PEAK_MST WHERE PEAk_LIST_ID IN (SELECT MAX(PEAK_LIST_ID) FROM TEST_DATA WHERE GRAPH_ID = '"+str(self.g[row])+"') ")
