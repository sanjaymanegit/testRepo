...
"color: rgb(170, 0, 0);")
        ...
        self.checkBox_7.setGeometry(QtCore.QRect(620, 120, 141, 31))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkBox_7.setText(_translate("MainWindow", "COF"))
    ...
    def load_data(self):      
        ...
        results=connection.execute("SELECT TENSILE_TEST_FLG,COMPRESS_TEST_FLG,TARE_TEST_FLG,flexural_test,QLSS_TEST,ILSS_TEST,COF_ACTIVE_FLG FROM GLOBAL_VAR") 
        for x in results:            
            if(str(x[0])=='ACTIVE'):
    ...
    def save_data(self):        
        ...
        if(self.checkBox_7.isChecked()):
            self.cof_flag='ACTIVE'
        else:
            self.cof_flag='DEACTIVE'
            
        connection = sqlite3.connect("tyr.db")          
        with connection:        
                cursor = connection.cursor()                    
                cursor.execute("UPDATE GLOBAL_VAR SET TENSILE_TEST_FLG = '"+str(self.tensile_flg)+"',COMPRESS_TEST_FLG='"+str(self.compress_flg)+"',TARE_TEST_FLG='"+str(self.tare_flag)+"' , flexural_test='"+str(self.flx_flag)+"',QLSS_TEST='"+str(self.qlss_flag)+"',ILSS_TEST='"+str(self.ilss_flag)+"',COF_ACTIVE_FLG='"+str(self.cof_flag)+"'") 
        connection.commit();
        connection.close()
